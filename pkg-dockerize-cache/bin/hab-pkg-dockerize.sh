...
docker_image() {
  # get dependency list for requested package
  local BUILD_PKGS="$@"
  for p in $BUILD_PKGS; do
    hab pkg install $p >/dev/null
    cat $(hab pkg path $p)/DEPS >> /tmp/_all_deps
  done
  local BUILD_DEPS=$(cat /tmp/_all_deps)
  rm -f /tmp/_all_deps

  for p in $BUILD_PKGS; do
    cat $(hab pkg path $p)/IDENT >> /tmp/_all_vers
  done
  local BUILD_PKGS_VERS=$(cat /tmp/_all_vers)
  rm -f /tmp/_all_vers

  env PKGS="$BUILD_DEPS" NO_MOUNT=1 hab-studio -r $DOCKER_CONTEXT/rootfs -t baseimage new
...
WORKDIR /
ADD rootfs /
RUN hab pkg install $BUILD_PKGS_VERS
...
EOT
  ...
  docker build --force-rm -t $version_tag .
